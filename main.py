...
class Piromain():
    ...
    def remove_background(self,img):
        ...
        #blur_gray = cv2.medianBlur(img, self.filterMask)
        #kernel = np.array([[1,0,1],[0,0,0],[1,0,1]])
        #img = cv2.filter2D(img,5, kernel)
        #blur_gray = np.uint8(img)
        #thresh = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, self.adaptiveThreshBlockSize, self.adaptiveThreshholdAddVal)
        #show(thresh)
        ...
        #show(thresh)
        #thresh = cv2.medianBlur(img, self.filterMask-2)
        #thresh = cv2.dilate(thresh, kernel=np.ones(shape=(self.erodeFirst, 1)))
        #show(img, cdst)
        return thresh
    ...
    def detect_lines(self,img, thresh):
        ...
        #show(thresh)
        #thresh = cv2.dilate(thresh, kernel=np.ones(shape=(1, self.dilateFirst)))
        #thresh = cv2.dilate(thresh, kernel=np.ones(shape=(self.dilateFirst, 1)))
        #show(thresh)
        thresh = cv2.erode(thresh, kernel=np.ones(shape=(3, 1)))
        #thresh = cv2.dilate(thresh, kernel=np.ones(shape=(1+self.erodeFirst-self.dilateFirst, 1)))
        thresh = cv2.bitwise_not(thresh)
        #show(thresh)
    ...
    def detectContours(self, img):
        ...
        for cont in contours:
            ...
            if (ar < 0.3* area and (borderUp <0.6*area or borderDown <0.6*area)) or (borderUp + borderDown < area *0.5) or (i==0 and borderUp > 2*area and borderDown*1.3 < borderUp) \
